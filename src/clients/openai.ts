...
export type {
  UserProfile as OpenAIUserProfile,
  ConversationContext as OpenAIConversationContext,
};
import {
  ContextFormatter,
  GPTMessage,
...
export class OpenAIService {
   ...
   */
  async generateWorkoutPlan(request: WorkoutGenerationRequest): Promise<any> {
    const prompt = PromptBuilder.buildWorkoutPrompt(
      request.userProfile,
      request.workoutType,
      request.duration,
      request.difficulty
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert fitness trainer. Generate workout plans in valid JSON format exactly as specified.",
        },
        {
          role: "user",
          content: prompt,
  ...
  async generateMealPlan(request: MealPlanRequest): Promise<any> {
    const prompt = PromptBuilder.buildMealPlanPrompt(
      request.userProfile,
      request.calories,
      request.macros,
      request.mealType,
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert nutritionist. Generate meal plans in valid JSON format exactly as specified.",
        },
        {
          role: "user",
          content: prompt,
  ...
  async analyzeProgress(
    ...
    const prompt = PromptBuilder.buildProgressAnalysisPrompt(
      userProfile,
      progressData,
      timeframe
    );
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert fitness analyst. Provide progress analysis in valid JSON format exactly as specified.",
        },
        {
          role: "user",
          content: prompt,
      ...
      messages: [
        {
          role: "system",
          content:
            "You are Nate, a supportive fitness coach. Create personalized, authentic motivational messages.",
        },
        {
          role: "user",
          content: prompt,
...
Provide only the exercise names, one per line, without numbering or additional text.`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a fitness expert. Provide concise exercise alternatives.",
        },
        {
          role: "user",
          content: prompt,
  ...
  async summarizeConversation(
    ...
    const contextSummary = ContextFormatter.summarizeContext(
      messages,
      userProfile,
      timeframe
    );
...
Keep it concise but informative for future reference.`;
  ...
  async generateContentVariations(
    content: string,
    type: "motivational" | "educational" | "instructional",
    count: number = 3
  ): Promise<string[]> {
...
Provide only the variations, numbered 1-${count}.`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a fitness content writer creating variations for testing.",
        },
        {
          role: "user",
          content: prompt,
   ...
   */
  async validateUserContent(
    content: string,
    type: "goal" | "preference" | "feedback" | "question"
  ): Promise<{
    isValid: boolean;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a content moderator for a fitness app. Ensure content is safe, appropriate, and helpful.",
        },
        {
          role: "user",
          content: prompt,
