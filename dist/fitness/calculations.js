...
export class FitnessCalculations {
     ...
     */
    static calculateBodyFatNavy(gender, height, // cm
    waist, // cm
    neck, // cm
     ...
     */
    static calculateLeanBodyMass(weight, bodyFatPercentage) {
        return Number((weight * (1 - bodyFatPercentage / 100)).toFixed(1));
    }
    /**
     * Get comprehensive body composition analysis
     */
    static getBodyComposition(metrics, measurements) {
        ...
        if (measurements?.waist && measurements?.neck) {
            try {
                bodyFatPercentage = this.calculateBodyFatNavy(metrics.gender, metrics.height, measurements.waist, measurements.neck, measurements.hip);
                leanBodyMass = this.calculateLeanBodyMass(metrics.weight, bodyFatPercentage);
            }
            catch (error) {
     ...
     */
    static calculateCalorieTarget(tdee, goal) {
        const adjustments = {
     ...
     */
    static calculateMacroTargets(calories, weight, goal, activityLevel) {
     ...
     */
    static getProteinTarget(goal, activityLevel) {
        const baseProtein = {
            weight_loss: 1.6,
            muscle_gain: 2.0,
            maintenance: 1.2,
     ...
     */
    static calculateTrainingWeight(oneRepMax, targetReps) {
        if (targetReps === 1)
            return oneRepMax;
        const percentages = {
            1: 1.0,
        ...
        const percentage = percentages[targetReps] ||
     ...
     */
    static calculateCaloriesBurned(weight, duration, // minutes
    activityMET) {
     ...
     */
    static getActivityMET(activity) {
        const metValues = {
            // Strength Training
     ...
     */
    static calculateProgressiveOverload(currentWeight, currentReps, targetReps, progressionType) {
     ...
     */
    static calculateRecoveryTime(workoutType, intensity, duration) {
     ...
     */
    static calculateHydrationNeeds(weight, activityLevel, climate = "temperate") {
...
export class NutritionCalculations {
     ...
     */
    static calculateMealTiming(workoutTime, // HH:MM format
    goal) {
     ...
     */
    static calculateMicronutrients(calories, age, gender, activityLevel) {
        const baseNeeds = {
            vitamin_d: {
                amount: 600,
                unit: "IU",
