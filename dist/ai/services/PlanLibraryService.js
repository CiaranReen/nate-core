export class PlanLibraryService {
    ...
    createBeginnerFullBodyPlan() {
        return {
            id: "beginner-full-body-1",
            name: "Beginner Full Body Foundation",
            description: "A balanced full-body program focusing on fundamental movement patterns and progressive overload.",
            targetAudience: {
    ...
    createBeginnerAdaptationTriggers() {
        return [
            {
    ...
    createIntermediatePPLPlan() {
        return {
            id: "intermediate-ppl-1",
            name: "Intermediate Push Pull Legs",
            description: "A balanced PPL split focusing on strength and hypertrophy with progressive overload.",
            targetAudience: {
    ...
    createAdvancedSplitPlan() {
        return {
            id: "advanced-split-1",
            name: "Advanced Body Part Split",
            description: "High-volume body part split for advanced lifters focusing on specialized hypertrophy and strength.",
            targetAudience: {
    ...
    async findBestMatchingPlan(profile, context, metrics) {
        const matchingPlans = this.library.plans.filter((plan) => this.isPlanSuitable(plan, profile, context, metrics));
    ...
    isPlanSuitable(plan, profile, context, metrics) {
        // Check experience level
        if (plan.targetAudience.experienceLevel !== profile.fitnessLevel) {
            return false;
        }
        // Check time commitment
        if (plan.targetAudience.timeCommitment.sessionsPerWeek >
            context.timeConstraints.availableDays.length) {
            return false;
        }
        // Check equipment requirements
        const hasRequiredEquipment = plan.targetAudience.equipment.every((eq) => context.equipmentAvailable.includes(eq));
    ...
    calculatePlanScore(plan, profile, context, metrics) {
        let score = 0;
        // Goal alignment (0-30 points)
        const goalAlignment = this.calculateGoalAlignment(plan, profile.fitnessGoals);
        ...
        const recoveryCompatibility = this.calculateRecoveryCompatibility(plan, metrics);
        score += recoveryCompatibility * 20;
        // Technical suitability (0-15 points)
        const technicalSuitability = this.calculateTechnicalSuitability(plan, profile);
        score += technicalSuitability * 15;
        // Equipment optimization (0-15 points)
        const equipmentOptimization = this.calculateEquipmentOptimization(plan, context);
        score += equipmentOptimization * 15;
        return score;
    }
    calculateGoalAlignment(plan, userGoals) {
        const matchingGoals = plan.targetAudience.goals.filter((goal) => userGoals.includes(goal));
        return (matchingGoals.length /
            Math.max(plan.targetAudience.goals.length, userGoals.length));
    }
    calculateTimeCompatibility(plan, context) {
        const sessionRatio = Math.min(context.timeConstraints.availableDays.length /
            plan.targetAudience.timeCommitment.sessionsPerWeek, 1);
        return sessionRatio;
    }
    calculateRecoveryCompatibility(plan, metrics) {
        const recoveryBuffer = Math.max(0, metrics.recoveryScore - plan.metrics.estimatedRecoveryDemand * 10) / 100;
        return Math.min(recoveryBuffer, 1);
    }
    calculateTechnicalSuitability(plan, profile) {
        ...
        const difficultyDelta = Math.abs(userTechnicalLevel - plan.metrics.technicalDifficulty);
        return Math.max(0, 1 - difficultyDelta / 10);
    }
    calculateEquipmentOptimization(plan, context) {
        const availableEquipment = new Set(context.equipmentAvailable);
        const requiredEquipment = new Set(plan.targetAudience.equipment);
        const intersection = new Set([...availableEquipment].filter((x) => requiredEquipment.has(x)));
