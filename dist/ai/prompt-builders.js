...
export class PromptBuilder {
     ...
     */
    static buildCoachingPrompt(userProfile, context, userMessage) {
...
Respond as Nate:`;
     ...
     */
    static buildWorkoutPrompt(userProfile, workoutType, duration, difficulty) {
...
Generate the workout now:`;
     ...
     */
    static buildMealPlanPrompt(userProfile, calories, macros, mealType, preferences) {
...
Create the meal plan now:`;
     ...
     */
    static buildProgressAnalysisPrompt(userProfile, progressData, timeframe) {
...
${progressData.workouts
            .map((w) => `- ${w.date.toDateString()}: ${w.type} (${w.duration}min) - ${w.completed ? "Completed" : "Incomplete"}${w.rating ? ` - Rating: ${w.rating}/10` : ""}`)
            .join("\n")}

Nutrition Tracking (${progressData.nutrition.length} days):
${progressData.nutrition
            .map((n) => `- ${n.date.toDateString()}: ${n.calories} cal, ${n.protein}g protein - ${n.adherence ? "On track" : "Off track"}`)
            .join("\n")}
...
${progressData.mood
            .map((m) => `- ${m.date.toDateString()}: ${m.score}/10${m.notes ? ` - ${m.notes}` : ""}`)
            .join("\n")}
...
Analyze the progress now:`;
     ...
     */
    static buildMotivationalPrompt(userProfile, context) {
